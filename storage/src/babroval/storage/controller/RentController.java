...
public class RentController {
	...
	private Dao<Rent> daoRent;
	private Dao<Storage> daoStorage;
	
	
	
	public RentController(RentView view, Dao<Rent> daoRent, Dao<Storage> daoStorage) {
		this.view = view;
		...
		view.getEnter().addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent ae) {
							...
							try {
								...
								daoRent.insert(
										new Rent(daoStorage.loadIdByStorageNumber((String) view.getComboNum().getSelectedItem()),
												InitDB.stringToDate(view.getTfDate().getText(), "dd-MM-yyyy"),
												InitDB.stringToDate("01-" + quarter + "-" + view.getLabelYear().getText(), "dd-MM-yyyy"), sum,
												view.getTfInf().getText()));
			
								view.getComboNum().setSelectedIndex(0);
								resetFrame();
								JOptionPane.showMessageDialog(view.getPanel(), "The payment has been successfully included", "Message",
	...
	private void updateFrame() {
		...
		if (view.getComboNum().getSelectedIndex() == 0) {
			...
			try {
				...
				String userName = daoUser.loadNameByStorageNumber((String) view.getComboNum().getSelectedItem());
				view.getTfName().setText(userName);

				Dao<Rent> daoRent = new RentDaoImpl();
				Date quarterPaid = daoRent
						.loadLastQuarterPaidByStorageNumber((String) view.getComboNum().getSelectedItem());
